...
class Poeple :
    ...
    def sign_in(self , email , name , user_type ):
        if name not in self.users :
            self.users[name] = User(email , name , user_type , generate_username(5) , generate_password(10))
        else : 
...
class User :
    ...
    def __init__(self , email , name , user_type , username , password):
        self.name = name
        self.email = email
